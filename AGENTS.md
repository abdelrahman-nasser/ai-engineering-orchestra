...
- Agent Execution Contract specification: `core/agent-execution-contract-specification.md` — authoritative semantic contract for immutable declarative intent binding one exact assigned external-inference Agent action to one already-resolved effective Task-wide Execution Mode; governs semantics if it conflicts with the Agent Execution Contract schema.
- Agent Execution Contract schema: `schemas/agent-execution-contract.schema.json` — machine-readable structural validation of the closed eleven-field Agent Execution Contract; semantically subordinate to `core/agent-execution-contract-specification.md`.
- Agent Execution Run specification: `core/agent-execution-run-specification.md` — authoritative semantic contract for immutable occurrence identity binding one caller-supplied opaque Run ID to one exact nested Agent Execution Contract; governs semantics if it conflicts with the Agent Execution Run schema.
- Agent Execution Run schema: `schemas/agent-execution-run.schema.json` — machine-readable structural validation of the closed two-field Agent Execution Run; semantically subordinate to `core/agent-execution-run-specification.md`.
- Agent Execution Authorization Grant specification: `core/agent-execution-authorization-grant-specification.md` — authoritative semantic contract for immutable positive authority artifacts bound to one exact Agent Execution Run, issuer namespace, authorization domain, and static validity interval; governs semantics if it conflicts with the Agent Execution Authorization Grant schema.
- Agent Execution Authorization Grant schema: `schemas/agent-execution-authorization-grant.schema.json` — machine-readable structural validation of the closed eight-field Agent Execution Authorization Grant; semantically subordinate to `core/agent-execution-authorization-grant-specification.md`.
- Agent Operation Tool Binding specification: `core/agent-operation-tool-binding-specification.md` — authoritative semantic contract for an immutable, provider-neutral selected-binding value joining one exact complete Agent Execution Run to one exact immutable configured Tool implementation identity without authority, discovery, admission, dispatch, or invocation; governs semantics if it conflicts with the Agent Operation Tool Binding schema.
- Agent Operation Tool Binding schema: `schemas/agent-operation-tool-binding.schema.json` — machine-readable structural validation of the closed two-field Agent Operation Tool Binding; semantically subordinate to `core/agent-operation-tool-binding-specification.md`.
- Agent Execution Dispatch Admission specification: `core/agent-execution-dispatch-admission-specification.md` — authoritative semantic contract for the immutable three-field positive authorization-consumption record whose operational provenance derives only from an authoritative store; governs semantics if it conflicts with the Agent Execution Dispatch Admission schema.
- Agent Execution Dispatch Admission schema: `schemas/agent-execution-dispatch-admission.schema.json` — machine-readable structural validation of the closed three-field Agent Execution Dispatch Admission; semantically subordinate to `core/agent-execution-dispatch-admission-specification.md`.
- Agent Execution Dispatch Admission Store specification: `core/agent-execution-dispatch-admission-store-specification.md` — authoritative semantic contract for trusted coordination, backend-neutral atomic Grant-consumption storage, exact historical retry, revocation serialization, and the supported bounded local SQLite realization.
