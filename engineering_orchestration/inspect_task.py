...
# Legacy source-location constant, never used as the installed CLI's project root.
...
def find_default_schema_path() -> Traversable | None:
    """Locate the tool-owned Task schema independently of the active project."""
    return schema_resource("task.schema.json")
...
def load_schema(schema_path: Traversable) -> dict[str, Any]:
...
def inspect_task(
    ...
    # Resolve schema
    active_schema_path: Traversable | None = None
