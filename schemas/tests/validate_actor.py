...
SCHEMA_PATH = REPO_ROOT / "schemas" / "actor.schema.json"
FIXTURE_DIR = REPO_ROOT / "schemas" / "tests" / "actor"
...
FIXTURE_CASES: dict[str, ExpectedFailure | None] = {
    "valid-human.yaml": None,
    "valid-agent.yaml": None,
    "valid-extra-competencies.yaml": None,
    "valid-unknown-competency.yaml": None,
    "invalid-missing-id.yaml": ExpectedFailure("required", (), "id"),
    "invalid-empty-id.yaml": ExpectedFailure("minLength", ("id",)),
    "invalid-missing-kind.yaml": ExpectedFailure("required", (), "kind"),
    "invalid-unsupported-kind.yaml": ExpectedFailure("enum", ("kind",)),
    "invalid-missing-competencies.yaml": ExpectedFailure(
        "required", (), "competencies"
    ),
    "invalid-empty-competencies.yaml": ExpectedFailure(
        "minItems", ("competencies",)
    ),
    "invalid-duplicate-competencies.yaml": ExpectedFailure(
        "uniqueItems", ("competencies",)
    ),
    "invalid-empty-competency.yaml": ExpectedFailure(
        "minLength", ("competencies", 0)
    ),
    "invalid-unknown-field.yaml": ExpectedFailure("additionalProperties", ()),
...
def main() -> int:
    print("AI Engineering Orchestra - Actor Schema Validation")
    print("=" * 63)
