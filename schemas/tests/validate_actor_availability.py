...
SCHEMA_PATH = REPO_ROOT / "schemas" / "actor-availability.schema.json"
FIXTURE_DIR = REPO_ROOT / "schemas" / "tests" / "actor-availability"
...
FIXTURE_CASES: dict[str, ExpectedFailure | None] = {
    "valid-available.yaml": None,
    "valid-unavailable.yaml": None,
    "valid-unknown.yaml": None,
    "invalid-missing-actor-id.yaml": ExpectedFailure(
        "required", (), "actor_id"
    ),
    "invalid-empty-actor-id.yaml": ExpectedFailure(
        "minLength", ("actor_id",)
    ),
    "invalid-missing-state.yaml": ExpectedFailure("required", (), "state"),
    "invalid-unsupported-state.yaml": ExpectedFailure("enum", ("state",)),
    "invalid-extra-field.yaml": ExpectedFailure("additionalProperties", ()),
    "invalid-provider-field.yaml": ExpectedFailure(
        "additionalProperties", ()
    ),
    "invalid-model-field.yaml": ExpectedFailure("additionalProperties", ()),
    "invalid-runtime-field.yaml": ExpectedFailure(
        "additionalProperties", ()
    ),
    "invalid-authorization-field.yaml": ExpectedFailure(
        "additionalProperties", ()
    ),
...
def main() -> int:
    print("AI Engineering Orchestra - Actor Availability Schema Validation")
    print("=" * 76)
