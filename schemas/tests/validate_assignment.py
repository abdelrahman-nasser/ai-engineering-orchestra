...
SCHEMA_PATH = REPO_ROOT / "schemas" / "assignment.schema.json"
FIXTURE_DIR = REPO_ROOT / "schemas" / "tests" / "assignment"
...
FIXTURE_CASES: dict[str, ExpectedFailure | None] = {
    ...
    "invalid-missing-task-id.yaml": ExpectedFailure("required", (), "task_id"),
    "invalid-missing-workflow-id.yaml": ExpectedFailure(
        "required", (), "workflow_id"
    ),
    "invalid-missing-stage-id.yaml": ExpectedFailure("required", (), "stage_id"),
    "invalid-missing-role-id.yaml": ExpectedFailure("required", (), "role_id"),
    "invalid-missing-actor-id.yaml": ExpectedFailure("required", (), "actor_id"),
    "invalid-empty-task-id.yaml": ExpectedFailure("minLength", ("task_id",)),
    "invalid-empty-workflow-id.yaml": ExpectedFailure(
        "minLength", ("workflow_id",)
    ),
    "invalid-empty-stage-id.yaml": ExpectedFailure("minLength", ("stage_id",)),
    "invalid-empty-role-id.yaml": ExpectedFailure("minLength", ("role_id",)),
    "invalid-empty-actor-id.yaml": ExpectedFailure("minLength", ("actor_id",)),
...
def main() -> int:
    print("AI Engineering Orchestra - Assignment Schema Validation")
    print("=" * 63)
