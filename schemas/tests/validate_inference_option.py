...
SCHEMA_PATH = REPO_ROOT / "schemas" / "inference-option.schema.json"
FIXTURE_DIR = REPO_ROOT / "schemas" / "tests" / "inference-option"
...
FIXTURE_CASES: dict[str, ExpectedFailure | None] = {
    ...
    "invalid-missing-option-id.yaml": ExpectedFailure(
        "required", (), "option_id"
    ),
    "invalid-empty-option-id.yaml": ExpectedFailure(
        "minLength", ("option_id",)
    ),
    "invalid-missing-provider-id.yaml": ExpectedFailure(
        "required", (), "provider_id"
    ),
    "invalid-empty-provider-id.yaml": ExpectedFailure(
        "minLength", ("provider_id",)
    ),
    "invalid-missing-model-id.yaml": ExpectedFailure(
        "required", (), "model_id"
    ),
    "invalid-empty-model-id.yaml": ExpectedFailure(
        "minLength", ("model_id",)
    ),
    "invalid-extra-field.yaml": ExpectedFailure("additionalProperties", ()),
    "invalid-provider-specific-field.yaml": ExpectedFailure(
...
def main() -> int:
    print("AI Engineering Orchestra - Inference Option Schema Validation")
    print("=" * 75)
