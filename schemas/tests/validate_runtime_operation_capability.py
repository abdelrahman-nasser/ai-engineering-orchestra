...
SCHEMA_PATH = REPO_ROOT / "schemas" / "runtime-operation-capability.schema.json"
FIXTURE_DIR = (
    REPO_ROOT / "schemas" / "tests" / "runtime-operation-capability"
)
...
FIXTURE_CASES: dict[str, FixtureExpectation] = {
    "valid-present.yaml": FixtureExpectation(),
    "valid-absent.yaml": FixtureExpectation(),
    ...
    "invalid-extra-resource-field.yaml": FixtureExpectation(
        ExpectedStructuralFailure("additionalProperties", ())
    ),
    "invalid-extra-tool-field.yaml": FixtureExpectation(
        ExpectedStructuralFailure("additionalProperties", ())
    ),
...
def structural_result(
    ...
    if expected is None:
        ...
        expected_failures = (
            expected if type(expected) is tuple else (expected,)
        )
...
def semantic_result(
    ...
    observation = RuntimeOperationCapabilityObservation(
        ...
        state=RuntimeOperationCapabilityState(document["state"]),
    )
    result = validate_runtime_operation_capability(
        (observation,),
        (AgentRuntimeOptionDefinition("known-runtime"),),
    )
...
def main() -> int:
    print(
        "AI Engineering Orchestra - Runtime Operation Capability Validation"
    )
    print("=" * 72)
    ...
    if (
        ...
        print("Runtime Operation Capability validation PASSED.")
        return 0
    print("Runtime Operation Capability validation FAILED.")
    return 1
