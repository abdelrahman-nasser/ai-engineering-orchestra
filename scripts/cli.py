...
This module is intentionally brand-neutral. Product naming exists only
in the thin entry-point script that imports and calls main().
"""
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    correct behavior both during development and after future global installation.
...
def cmd_tasks(args: argparse.Namespace, project_root: Path) -> int:
    ...
    Delegates to existing Task inventory capability in scripts/list_tasks.py.
    """
    from scripts.list_tasks import discover_tasks, format_inventory_table
...
def cmd_inspect(args: argparse.Namespace, project_root: Path) -> int:
    ...
    """
    from scripts.inspect_task import format_report, inspect_task
    from scripts.list_tasks import discover_tasks
...
def cmd_verify(args: argparse.Namespace, project_root: Path) -> int:
    ...
    Delegates to existing repository preflight capability in scripts/verify_repo.py.
    """
    from scripts.verify_repo import format_preflight_output, run_preflight
